...
namespace Kakegurui.Core
{
    ...
    public static class ByteFormatter
    {
        ...
        public static int Deserialize(object value, List<byte> buffer,int offset=0)
        {
            ...
            {
                LogPool.Logger.LogError((int)LogEvent.系统,ex,$"{offset} {ByteConvert.ToHex(buffer?.ToArray())}");
                return -1;
            }
        ...
        private static Tuple<object,int> ToValue(Type type,List<byte> buffer,int offset)
        {
            ...
            if (type.IsArray)
            {
                ...
                var elemnetType = type.GetElementType();
                if (elemnetType == null)
                {
                    return new Tuple<object, int>(null, -1);
                }
                Array array = Array.CreateInstance(elemnetType, length);
                int size = lt.Item2;
            ...
            if (type.FullName != null && type.FullName.StartsWith("System.Collections.Generic.List"))
            {
                ...
                System.Collections.IList list =(System.Collections.IList)constructor.Invoke(new object[]{});
                for (int i = 0; i < length; ++i)
                {
                    var (item1, item2) = ToValue(type.GetGenericArguments()[0], buffer, offset + size);
                    if (item2 == -1)
                    {
                        return new Tuple<object, int>(null, -1);
                    }
